...
Issues and specs for this repo live as GitHub issues in `eclinelu/roof-pipeline`.
...
## Relationship to `decisions/` and `STATE.md`

The issue tracker is NOT the project record. `decisions/` (append only, one file
per entry) and `STATE.md` (overwritten, true right now) remain the authority, per
the precedence rule at the top of `CLAUDE.md`. GitHub issues hold work items:
what to do next, what is blocked, what a spec asks for. When an issue resolves
something that counts as a decision, log the decision through the decision-log
skill. Do not treat a closed issue as a logged decision.
