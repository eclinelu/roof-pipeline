...
- **Active blocker:** the deliberate pre-registration freeze (Emmett's act). Field visit already happened; the freeze must now record what it found honestly (see below) before any real-world conversion.
- **Site findings (2026-07-15, roof visit):** (1) the fallback ridge span `length:r6,7` was tape-measured on site BEFORE the freeze, so this is NOT a scale-blind pre-registration; the honest claim is only that the cloud-unit geometry is locked before any real-world conversion (the tape value and the multiplier live in the later comparison file, never in the freeze). (2) the predicted rank-1 primary `lines:j0,3-j0,5` was found NON-EXISTENT on the roof: dormer points corrupted facet 0's junctions, so it is recorded as predicted-but-refuted, the frozen candidate left as the pipeline produced it. (3) about 8 dormers were not segmented and contaminate specific per-facet numbers (see the 2026-07-15 dormer decision entry); which facets could not be cleanly attributed (facets 4 and 5 alone are near-clean).
...
- **Next action:** Emmett runs `python scripts/preregister.py C:\odm\datasets\big_house --candidate lines:j0,3-j0,5 --fallback length:r6,7 --notes-file C:\odm\datasets\big_house\freeze_context.txt` (drop `--dry-run`) and COMMITS the JSON: that commit is the pre-registration. Then the real-world scale (the taped ridge value and multiplier), the comparison, and any further roof/inclinometer readings all go in a NEW comparison file, never editing the frozen one.
