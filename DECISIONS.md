...
- **Phase:** Roof-derived scale recon BUILT, run, and PRE-REGISTERED. Spec `docs/superpowers/specs/2026-07-14-roof-scale-recon-design.md`, plan `docs/superpowers/plans/2026-07-14-roof-scale-recon.md`. The pre-registration is frozen and committed by Emmett as `6c2e614` (`reports/big_house/preregistered-2026-07-15.json`): 8 facets, total 313.188 cu^2, candidate `length:r6,7` (10.808 cu), fallback `lines:r1,3-e1` (6.727 cu), verbatim context field, no real-world units. 46 tests green.
- **Active blocker:** none in code. Next is real-world: measure/confirm the taped spans, compute scale, and score the frozen file in a NEW comparison file. Separately, the deferred dormer-detection effort (see the 2026-07-15 dormer entry) is open work whenever it is picked up.
...
- **Last thing verified working (2026-07-15):** `roof_recon.py` full run on big_house: 8 facets, 18 intersection lines (4 ridges at az 88.6/112.6/125.8/178.5), monotone eave brackets, 24 ranked candidates; segmentation bit-identical across fresh processes. `preregister.py --dry-run` reproduces the 7a totals (313.19 cu^2) and now embeds a verbatim `context` field from `--notes-file` (draft at `C:\odm\datasets\big_house\freeze_context.txt`) recording the three site findings as metadata, without altering any frozen number. 46 tests green.
- **Next action:** (real-world, post-freeze) In a NEW comparison file (never editing `preregistered-2026-07-15.json`): convert the frozen candidate span `length:r6,7` (10.808 cu) against its on-site tape reading to get the scale multiplier, apply it to per-facet and total areas, and record the measured GPS scale error. Use `lines:r1,3-e1` as the fallback if the ridge reading is unusable. Score per-facet pitch against inclinometer readings, remembering only facets 4 and 5 are dormer-clean. Then decide whether to open the deferred dormer-detection effort.
