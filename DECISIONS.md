...
- **Results (run 2 scaling, 40.454 in/cu):** total roof area 3,559 ft^2; the area claim is "scale confirmed by an independent length" (no measured area exists for big_house, permanently). Pitch: max |error| 2.19 deg, PASS at the 3 deg hypothesis bound, not at 2 deg; systematic +1.83 deg bias (pipeline steeper than inclinometer at ALL azimuths, so not residual tilt; instrument-vs-surface question, open). Facets 6/7 scored against 90-minus readings per the interpretation entry. GPS scale error +2.75% under run 2. Comparison files: `comparison-2026-07-15-scored-2026-07-18.json`, `comparison-2026-07-18-scored-2026-07-18.json`. 47 tests green.
