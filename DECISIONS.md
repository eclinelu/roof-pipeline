...
- **Next action:** Emmett runs `python scripts/preregister.py C:\odm\datasets\big_house --candidate length:r6,7 --fallback lines:r1,3-e1 --notes-file C:\odm\datasets\big_house\freeze_context.txt` (drop `--dry-run`) and COMMITS the JSON: that commit is the pre-registration. Both candidate ids and the context field verified by dry-run 2026-07-15. Then the real-world scale (the taped ridge value and multiplier), the comparison, and any further roof/inclinometer readings all go in a NEW comparison file, never editing the frozen one.
